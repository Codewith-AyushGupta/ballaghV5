...
function ProductGridView(props) {
  ...
  const getProductsForStore = () => {
    try {
      let ProductDatabaseServiceInstance = new ProductDatabaseService(
        productDatabase
      );
      let StoreDatabaseServiceInstance = new StoreDatabaseService(
        storeDatabase
      );

      const filteredProducts =
        ProductDatabaseServiceInstance.getDefaultProductsVariantArrayByStoreSlug(
          storeSlug
        );
      const filteredStore =
        StoreDatabaseServiceInstance.getStoreByStoreSlug(storeSlug);
        ...
        <div className="container">
          <div className="row main-content-wrap gutter-lg">
            <div className="main-content">
                ...
                {defaultProductProductVariant.map((item) => (
                  <div className="product-wrap" key={"shop-" + item.slug}>
                    <CardItem
                      image1={
                        item.bundleItems
                          ? item.bundleDefaultImage.pictures[0].url
                          : item.pictures[0].url
                      }
                      image2={
                        item.bundleItems
              ...
              {defaultProductProductVariant.length === 0 ? (
                <h2 className=" text-align-center container post-title">
