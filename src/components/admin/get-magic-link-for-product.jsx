...
function GetMagicLinkForProduct() {
    const {
        tenantConfiguration,
    ...
    const generateMagicLink = async (e) => {
        ...
        try {
            let response = await axios.post(
                ...
                {
                    pipelineName: tenantConfiguration.REACT_APP_GET_ADMIN_ACCESS_MAGIC_PIPELINE_NAME_FOR_DATABASE_UPDATE,
                    pipelineParams: [
                        { name: 'email', value: formDetails.email },
                        { name: 'pageUrl', value: window.location.origin+'/upload-product-file' },
                        { name: 'fileSuffix', value: 'productDatabase' }
                    ],
                },
                {
                    headers: {
...
export default GetMagicLinkForProduct
